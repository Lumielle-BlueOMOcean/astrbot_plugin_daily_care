...
- 正式发布版本号采用语义化版本（metadata + @register 两处一致）：bug 修复升补丁位，新功能升次版本位。开发阶段普通 commit、bugfix、review fix、CI 修复和连续迭代默认不修改正式版本号；只在正式发布准备阶段统一提升版本并创建发布提交。
- README 只放用户实际需要的功能、安装、兼容性和使用说明；`CHANGELOG.md` 作为完整正式版本历史记录，README 同样直接展示全部正式版本的面向用户更新日志；新版本发布时必须同步更新两份文档。内部调试过程、审查记录和技术流水写入内部开发日志，不复制到 README。
- 发布前七查：编译 / 测试全绿 / schema 对齐 / hint ≤25字 / 个性化残留零 / 版本号统一 / 残留清理与包校验。
- 每次发布生成纯净安装包（不含 data/、运行时状态、备份、__pycache__），部署时保留本机已有 data/ 与配置。
- 许可证：MIT。
